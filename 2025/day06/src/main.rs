...
fn sum_numbers(operator_vec: &[String]) -> Result<u64> {
    let mut sum = 0;
...
fn prod_numbers(operator_vec: &[String]) -> Result<u64> {
    let mut prod = 1;
...
fn build_column(char_lines: &[Vec<char>], i: usize) -> Vec<char> {
    let mut column: Vec<char> = Vec::new();
    for char_line in char_lines {
        column.push(char_line[i]);
    }
    column
}

fn get_number(column: &[char]) -> Result<u64> {
