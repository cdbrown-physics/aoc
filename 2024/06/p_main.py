...
def part_two_main(args):
    row, path_walked_row, guard = args
    stuck_count = 0
    for spot in path_walked_row:
...
def PartTwo(guard: Guard):
    ...
    tasks = [(row, path_walked[row], guard) for row in path_walked]
    start = dt.now()
    with Pool(cpu_count()) as pool:
        results = pool.map(part_two_main, tasks)
    answer = sum(results)
